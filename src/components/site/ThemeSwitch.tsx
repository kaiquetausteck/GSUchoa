...
export function ThemeSwitch({
  ...
  return (
    <button
      ...
      aria-pressed={!isDark}
      className={`relative inline-grid grid-cols-2 items-center rounded-full border border-outline-variant/20 bg-surface-container-high p-1 text-on-surface transition-colors duration-300 hover:border-primary/35 ${
        compact ? "h-10 w-[4.5rem]" : "h-11 w-20"
      }`}
      onClick={toggleTheme}
      type="button"
    >
      ...
      <span className="relative z-10 flex h-full w-full items-center justify-center">
        <SunMedium
          className={`h-4 w-4 transition-colors ${isDark ? "text-on-surface-variant" : "text-white"}`}
        />
      </span>
    </button>
  );
