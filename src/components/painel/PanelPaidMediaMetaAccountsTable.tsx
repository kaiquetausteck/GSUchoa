...
type PanelPaidMediaMetaAccountsTableProps = {
  isLoading: boolean;
  items: PanelMetaAdAccountRecord[];
  onOpenDashboard: (item: PanelMetaAdAccountRecord) => void;
};

function formatAccountStatus(value: number | null) {
  if (value === null) {
...
export function PanelPaidMediaMetaAccountsTable({
  isLoading,
  items,
  onOpenDashboard,
}: PanelPaidMediaMetaAccountsTableProps) {
  if (isLoading) {
    return (
  ...
  if (items.length === 0) {
    return (
      <div className="panel-card rounded-[1.75rem] border border-dashed px-6 py-12 text-center">
        <p className="text-sm font-semibold text-on-surface">Nenhuma conta Meta disponível</p>
        <p className="mt-2 text-sm leading-relaxed text-on-surface-variant">
          Quando o backend retornar contas de anúncio válidas, elas aparecerão aqui para abrir o dashboard.
        </p>
      </div>
    );
  }
        ...
        <table className="min-w-full text-left text-sm">
          <thead className="panel-card-muted border-b border-outline-variant/12">
            <tr className="text-[11px] uppercase tracking-[0.18em] text-on-surface-variant">
              <th className="px-6 py-4 font-semibold">Conta Meta</th>
                ...
                <td className="min-w-[24rem] px-6 py-5">
                  <div className="flex items-center gap-4">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="mt-1 truncate text-xs text-on-surface-variant">{item.adAccountId}</p>
                    </div>
                  </div>
                </td>
                <td className="px-6 py-5 text-on-surface-variant">
                  {item.currency || "Não informada"}
                </td>
                <td className="px-6 py-5">
                  <span className="inline-flex items-center gap-2 text-on-surface-variant">
                    <Clock3 className="h-3.5 w-3.5" />
                    {item.timezoneName || "Não informado"}
                  </span>
                </td>
                <td className="px-6 py-5">
