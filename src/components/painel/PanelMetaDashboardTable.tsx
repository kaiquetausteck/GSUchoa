...
import {
  PANEL_META_DASHBOARD_TABLE_LEVEL_VALUES,
  type PanelMetaDashboardTableLevel,
  type PanelMetaDashboardTableRowRecord,
} from "../../services/painel/meta-dashboard-api";

type PanelMetaDashboardTableProps = {
  isLoading: boolean;
  level: PanelMetaDashboardTableLevel;
  onLevelChange: (value: PanelMetaDashboardTableLevel) => void;
  rows: PanelMetaDashboardTableRowRecord[];
};

const TABLE_LEVEL_LABELS: Record<PanelMetaDashboardTableLevel, string> = {
  ad: "Anúncios",
  adset: "Conjuntos",
  campaign: "Campanhas",
};
...
export function PanelMetaDashboardTable({
  isLoading,
  level,
  onLevelChange,
  rows,
}: PanelMetaDashboardTableProps) {
  return (
      ...
      <div className="flex flex-col gap-4 border-b border-outline-variant/10 pb-5 lg:flex-row lg:items-start lg:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-2 max-w-2xl text-sm leading-relaxed text-on-surface-variant">
            Acompanhe os dados detalhados da conta Meta no nível mais útil para a análise atual.
          </p>
        </div>
        ...
        <div className="w-full max-w-[13rem]">
          <AppSelect
            ...
            onChange={(event) => onLevelChange(event.target.value as PanelMetaDashboardTableLevel)}
            value={level}
          >
            {PANEL_META_DASHBOARD_TABLE_LEVEL_VALUES.map((item) => (
