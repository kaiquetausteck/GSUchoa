...
type PanelPaidMediaMetaClientsTableProps = {
  isLoading: boolean;
  items: PanelClientSummaryRecord[];
  onOpenDashboard: (item: PanelClientSummaryRecord) => void;
};
...
export function PanelPaidMediaMetaClientsTable({
  isLoading,
  items,
  onOpenDashboard,
}: PanelPaidMediaMetaClientsTableProps) {
  if (isLoading) {
    return (
    ...
    return (
      <div className="panel-card rounded-[1.75rem] border border-dashed px-6 py-12 text-center">
        ...
        <p className="mt-2 text-sm leading-relaxed text-on-surface-variant">
          Ajuste a busca para localizar um cliente e abrir o dashboard operacional da Meta.
        </p>
      </div>
    );
  }
