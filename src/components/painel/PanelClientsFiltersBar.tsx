...
import type { PanelClientSort } from "../../services/painel/clients-api";
import { AppInput } from "../shared/ui/AppInput";
import { AppSelect } from "../shared/ui/AppSelect";

type PanelClientsFiltersBarProps = {
  ...
  onSortChange: (value: PanelClientSort) => void;
  perPage: number;
  publishedValue: "all" | "published" | "draft";
  searchValue: string;
  sortValue: PanelClientSort;
};

const SORT_OPTIONS: Array<{ label: string; value: PanelClientSort }> = [
  ...
  const featuredLabel =
    featuredValue === "featured"
      ...
      : featuredValue === "regular"
        ? "Sem destaque"
        : "Qualquer destaque";
  const sortLabel = SORT_OPTIONS.find((option) => option.value === sortValue)?.label ?? "Ordem crescente";
      ...
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div className="min-w-0 flex-1">
          <AppInput
            ...
            placeholder="Buscar por nome, slug ou website"
            value={searchValue}
            ...
            <Popover.Portal>
              ...
              >
                <div className="space-y-4">
                  ...
                  <AppSelect
                    label="Ordenação"
                    onChange={(event) => onSortChange(event.target.value as PanelClientSort)}
                    value={sortValue}
                  >
