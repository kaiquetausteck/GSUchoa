...
import {
  deletePanelMetaConnection,
  getPanelMetaConnectLink,
  getPanelMetaConnectionStatus,
  validatePanelMetaConnection,
  type PanelMetaConnectionDetailsRecord,
...
function toStatusRecord(details: PanelMetaConnectionDetailsRecord): PanelMetaConnectionStatusRecord {
  return {
...
function MetaStatusBadge({
  ...
  return (
    <span
      className={`inline-flex items-center gap-2 rounded-full border px-3.5 py-1.5 text-xs font-semibold ${getPanelMetaStatusBadgeClassName(
        status,
      )}`}
    >
...
export function MetaTab({
  ...
  const reconnectLabel = statusRecord?.status === "NOT_CONNECTED" ? "Conectar Meta" : "Reconectar";
    ...
    try {
      ...
      const message =
        error instanceof Error
          ? error.message
          : "Não foi possível carregar o status da integração Meta agora.";
  ...
  const feedbackBanner = useMemo(() => {
    if (callbackError) {
      return (
        <MetaFeedbackBanner
          ...
          title="A conexão com a Meta não foi concluída"
        />
      );
    }
    ...
    if (callbackConnected) {
      return (
        <MetaFeedbackBanner
          kind="success"
          message="A autenticação da conta Meta foi concluída com sucesso."
  ...
  const handleConnect = async () => {
    ...
    try {
      const response = await getPanelMetaConnectLink(token);
  ...
  const handleValidateConnection = async () => {
    ...
    try {
      const nextDetails = await validatePanelMetaConnection(token);
      setStatusRecord(toStatusRecord(nextDetails));
      toast.success({
        title: "Conexão verificada",
        description: "O backend validou a integração da Meta e atualizou o status.",
      });
    } catch (error) {
      const message =
        error instanceof Error
          ? error.message
          : "Não foi possível validar a integração com a Meta agora.";
  ...
  const handleDisconnect = async () => {
    ...
    try {
      await deletePanelMetaConnection(token);
      setDisconnectDialogOpen(false);
      toast.success({
        title: "Meta desconectada",
        description: "A conexão central da Meta foi removida com sucesso.",
      });
      ...
      const message =
        error instanceof Error
          ? error.message
          : "Não foi possível desconectar a integração Meta agora.";

      toast.error({
        title: "Falha ao desconectar a Meta",
        description: message,
      });
    } finally {
      ...
      <section className="w-full space-y-5">
        ...
        <section className="panel-premium-card overflow-hidden rounded-[2.2rem] border p-8">
          <div className="flex min-h-[18rem] flex-col items-center justify-center text-center">
            ...
            <p className="mt-3 max-w-lg text-sm leading-relaxed text-on-surface-variant">
              {loadError || "O backend não retornou um status válido para a integração Meta."}
            </p>
            <button
  ...
  return (
    <section className="w-full space-y-5">
        ...
        <div className="relative grid gap-8 px-6 py-7 md:px-8 md:py-8 2xl:px-10 2xl:py-10 xl:grid-cols-[minmax(0,1.7fr)_minmax(22rem,28rem)] xl:items-start 2xl:grid-cols-[minmax(0,1.95fr)_minmax(24rem,30rem)]">
          <div>
            <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
              ...
              <MetaStatusBadge status={statusRecord.status} />
            </div>

            <div className="mt-8 grid gap-4 md:grid-cols-3">
              <MetaMetric label="Status atual" value={getPanelMetaStatusLabel(statusRecord.status)} />
              <MetaMetric label="Última validação" value={formatDateTime(statusRecord.lastValidatedAt)} />
              <MetaMetric label="Expiração do token" value={formatDateTime(statusRecord.expiresAt)} />
            </div>
          ...
          <aside className="panel-integration-side rounded-[2rem] border p-5">
            ...
            <p className="mt-2 text-sm leading-relaxed text-on-surface-variant">
              {hasConnection
                ...
                : "A integração ainda não está ativa. Inicie a conexão para liberar o uso operacional da Meta no painel."}
            </p>
            ...
            <div className="panel-integration-note mt-6 rounded-[1.5rem] border px-4 py-4">
              ...
              <p className="mt-2 text-sm leading-relaxed text-on-surface-variant">
                {getPanelMetaStatusDescription(statusRecord.status)}
              </p>
            </div>

            {panelMetaStatusNeedsReconnect(statusRecord.status) ? (
              <p className="mt-4 text-xs leading-relaxed text-on-surface-variant">
                O backend identificou que uma nova autorização pode ser necessária para manter a operação da Meta estável.
              </p>
            ) : null}
          </aside>
        </div>
      ...
      <ConfirmDialog
        ...
        confirmLabel="Desconectar Meta"
        description="Essa ação remove a conexão central da Meta da agência. Você poderá reconectar depois se necessário."
        ...
        title="Deseja desconectar a Meta?"
      />
    </section>
  );
}
